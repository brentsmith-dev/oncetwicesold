...
BarsOfStuff.protocol = 'https:' == document.location.protocol ? 'https:' : 'http:';
...
BarsOfStuff.STYLES = [
  ...
  {name:'worm', img: 'worm.png', height: 40, headImg: 'worm_head.png', headWidth: 26},
  {name:'horse', img: 'horse.png', height: 26}
];
...
BarsOfStuff.prototype.draw = function(data, options) {
  ...
  for (var i = 0; i < bars.length; i++) {
    ...
    html.push('<tr><td class="barsofstuff-label" height="', chartStyle.height, '">',
  ...
  var headWidth = chartStyle.headWidth || 0;
  ...
  // Draw the bars
  var heads = ['heads-brent.png','heads-luke.png','heads-scott.png','heads-gazheek.png','heads-aaron.png','train_head.png'];
  for (var i = 0; i < bars.length; i++) {
    var bar = bars[i];
    var headImg = heads[i];
    ...
    html.push('<div style="width: ',
        ...
        36,
        'px; background: url(',
        BarsOfStuff.IMGPATH, chartStyle.img,
        ') repeat top right;"></div>');
    if (headWidth > 0) {
      html.push('</td><td><img src="',
          BarsOfStuff.IMGPATH, headImg,
          '" width="', headWidth, '" height="',
          chartStyle.height, '" /></td></tr></table>');
    }
